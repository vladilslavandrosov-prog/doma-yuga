...
// Простой rate limit для /api/auth/login: 5 попыток за 15 минут с одного IP
...
export async function registerRoutes(
  ...
  app.post("/api/admin/clients", requireAdmin, async (req, res) => {
    ...
    const user = await storage.createUser({ username, password: await hashPassword(password), role: "client", clientId: client.id });
    if (projectId) {
