...
- Sidebar-навигация с 10 разделами (включая «О компании», «Наши преимущества», «Плановая смета», «Выполнение работ», «Галерея»)
- Сметы разделены на: «Плановая смета» (все позиции с табами Работы/Материалы/Транспорт) и «Выполнение работ» (выполненные + в работе, с видами «По дням», «Общая» и «По видам работ»; иконки рабочий/выходной день, нерабочие дни с причинами)
- Выполнение работ: админ может создавать/редактировать/удалять записи, загружать фото к каждой услуге. Клиенты — только просмотр. Вкладка «Общая» показывает все записи по датам со статусами. Вкладка «По видам работ» группирует позиции по полю workGroup
- Фото услуг: таблица estimateItemPhotos (estimateItemId, url), загрузка через /api/admin/estimate-item-photos/upload
- Публичная галерея (/gallery): отдельная от «Выполнения работ», собственная таблица gallery_photos с полями (id, url, caption, category). Фильтрация по категории. Lightbox. Админ может загружать/удалять фото через /api/admin/gallery/upload и /api/admin/gallery/:id
- Сообщения дня (dayComments): таблица day_comments (id, projectId, date, text, createdAt). Админ может создать/редактировать/удалить сообщение к любому рабочему дню. При создании/редактировании отправляется уведомление в Telegram. Отображается в «По дням» и «Общая» видах на странице «Выполнение работ»
- workGroup — nullable text поле в estimateItems; используется для группировки в «По видам работ» и в Галерее
